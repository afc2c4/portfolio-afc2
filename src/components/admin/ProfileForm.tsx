...
import { X, Plus, Save, Upload, User, Move, Settings2, ZoomIn } from 'lucide-react';
      ...
      <Card className="shadow-md">
          ...
          <div className="flex flex-col lg:flex-row gap-12 items-start">
                  ...
                  {!isDragging && formData.avatarUrl && (
                    <div className="absolute inset-0 bg-black/30 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center text-white pointer-events-none space-y-2">
                      <div className="bg-black/60 px-3 py-1 rounded-full text-[10px] font-bold uppercase flex items-center gap-2">
                        <Move className="w-3 h-3" /> Arraste
                      </div>
                      <div className="bg-black/60 px-3 py-1 rounded-full text-[10px] font-bold uppercase flex items-center gap-2">
                  ...
                  <AccordionItem value="adjustments" className="border-none">
                    <AccordionTrigger className="py-2 hover:no-underline">
                      <div className="flex items-center gap-2 text-xs font-bold text-muted-foreground uppercase tracking-wider">
                        <Settings2 className="w-3.5 h-3.5" /> Ajustes Finos
                      </div>
                    </AccordionTrigger>
                    <AccordionContent>
                      <div className="w-full space-y-6 bg-muted/30 p-4 rounded-xl border mt-2">
                        <div className="space-y-3">
                          <Label className="text-xs">Zoom: {(formData.avatarSettings?.scale || 1).toFixed(1)}x</Label>
                          <Slider value={[formData.avatarSettings?.scale || 1]} min={1} max={4} step={0.1} onValueChange={([val]) => updateAvatarSetting('scale', val)} />
                        </div>
                        <div className="space-y-3">
                          <Label className="text-xs">Horizontal: {Math.round(formData.avatarSettings?.x || 0)}%</Label>
                          <Slider value={[formData.avatarSettings?.x || 0]} min={-100} max={100} step={1} onValueChange={([val]) => updateAvatarSetting('x', val)} />
                        </div>
                        <div className="space-y-3">
                          <Label className="text-xs">Vertical: {Math.round(formData.avatarSettings?.y || 0)}%</Label>
                          <Slider value={[formData.avatarSettings?.y || 0]} min={-100} max={100} step={1} onValueChange={([val]) => updateAvatarSetting('y', val)} />
                        </div>
                      </div>
                    </AccordionContent>
                  </AccordionItem>
            ...
            <div className="flex-grow space-y-6 w-full">
              ...
              <div className="space-y-2">
                ...
                <Textarea id="bio" value={formData.bio} onChange={(e) => setFormData({ ...formData, bio: e.target.value })} className="min-h-[150px]" required />
              </div>
            </div>
          </div>
        </CardContent>
        <CardFooter className="bg-muted/30 pt-6">
          <Button type="submit" className="ml-auto flex items-center gap-2">
            <Save className="w-4 h-4" /> Salvar Perfil Dev
          </Button>
        </CardFooter>
      </Card>
    </form>
